...
export function useApproveToken() {
  ...
  const approve = async (spender: `0x${string}`, amount: bigint) => {
    if (!address)
      throw new Error("Token contract not deployed on this network");
...
export function useHasPurchasedProduct(productId: bigint) {
  ...
  return useReadContract({
    ...
    args: userAddress && productId ? [userAddress] : undefined,
    query: {
      enabled: !!contractAddress && !!userAddress,
    },
  });
...
export function usePurchaseProduct() {
  ...
  const purchaseProduct = async (productId: any) => {
    ...
    console.log("[v0] Purchasing product222:", productId, address);
    return writeContract({
      address,
      abi: MARKETPLACE_V2_ABI,
